      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          if (err.code === err.PERMISSION_DENIED) {
            error.value = 'Location access denied. Please enable location permissions in your browser settings.'
          } else if (err.code === err.POSITION_UNAVAILABLE) {
            error.value = 'Location unavailable. Your device could not determine its position.'
